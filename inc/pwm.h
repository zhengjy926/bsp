...
struct pwm_state {
	...
	uint32_t duty_cycle;            /**< Duty cycle, in nanoseconds */
 ...
 */
struct pwm_device {
    const char *name;           /**< 设备标识符 */
    uint32_t flags;             /**< 标志位 */
    uint32_t channel;           /**< per-chip relative index of the PWM device */
    struct pwm_controller *chip;      /**< PWM chip providing this PWM device */
 ...
 */
struct pwm_ops {
	int (*set)(struct pwm_controller *chip, struct pwm_device *pwm,
                 const struct pwm_state *state);  /**< 设置PWM状态 */
    int (*enable)(struct pwm_controller *chip, struct pwm_device *pwm);  /**< 使能PWM输出 */
    int (*disable)(struct pwm_controller *chip, struct pwm_device *pwm); /**< 禁用PWM输出 */
    int (*capture)(struct pwm_controller *chip, struct pwm_device *pwm,
...
int pwm_set_period(struct pwm_device *pwm, uint32_t period);
int pwm_set_duty(struct pwm_device *pwm, uint32_t duty);
int pwm_set_polarity(struct pwm_device *pwm, uint8_t polarity);

//int pwm_set_config(struct pwm_device *pwm, struct PwmConfig *config);
